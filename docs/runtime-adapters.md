...
codex-runtime.sh preview <plan.json> --workspace-root <absolute-path> \
  --session-root <absolute-path> --codex-bin <absolute-path> [--max-concurrency 1|2]
codex-runtime.sh execute <plan.json> --workspace-root <absolute-path> \
  --session-root <absolute-path> --codex-bin <absolute-path> --approve <sha256-token> \
  [--max-concurrency 1|2]
```
...
spectra runtime codex preview <plan.json> --workspace-root <absolute-path> \
  --session-root <absolute-path> --codex-bin <absolute-path>
...
`preview` binds the plan, canonical roots, Codex executable and version, explicit
...
The desktop Codex runtime may still discover and describe skills installed under
the user's Codex home even when user configuration, plugins, and skill search are
disabled. In local testing that added substantial fixed prompt context before the
review input was considered. Until Spectra has a dedicated, authenticated minimal
Codex home (or an App Server/SDK boundary with equivalent isolation), live
execution should be treated as an experimental compatibility path rather than a
cost-efficient default. Preview and fake-runtime verification are unaffected.

Relevant upstream references: [Codex non-interactive mode](https://learn.chatgpt.com/docs/non-interactive-mode)
and the [Codex configuration reference](https://learn.chatgpt.com/docs/config-file/config-reference).
...
The next gate is a dedicated minimal Codex execution profile that does not load the
user's unrelated skills, followed by a synthetic one-worker end-to-end smoke and
